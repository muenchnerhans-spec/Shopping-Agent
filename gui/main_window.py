...
Tabs:
  ...
  4. Settings   – Scrape interval and notification preferences.
"""
...
class MainWindow:
    ...
    def _build_settings_tab(self) -> None:
        ...
        inner = ttk.LabelFrame(frame, text="Scheduler")
        inner.pack(fill=tk.X, padx=16, pady=12)

        ttk.Label(inner, text="Prüfintervall (Minuten):").grid(
            row=0, column=0, padx=8, pady=6, sticky=tk.W
        )
        self._spin_interval = tk.Spinbox(inner, from_=1, to=1440, width=6)
        ...
        ttk.Button(inner, text="Übernehmen", command=self._on_apply_interval).grid(
